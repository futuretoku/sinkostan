...
<div class="bg-[#f8fafc] min-h-screen p-4 md:p-8 text-slate-800">
    <div class="max-w-6xl mx-auto space-y-6">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-6 max-w-4xl mx-auto">
                @forelse($rooms as $room)
                    @php
                        ...
                        // GUNAKAN BASE64 BIAR DATA AMAN SAAT DIKIRIM KE JS
                        $safeImagesData = base64_encode(json_encode($processedImages));
                    @endphp

                    {{-- PERBAIKAN: Gunakan tanda kutip tunggal untuk membungkus argumen --}}
                    <div onclick="openModal('{{ $room->id }}', '{{ $room->room_number }}', '{{ $hargaFormatted }}', '{{ $config['label'] }}', '{{ $room->type }}', '{{ $safeImagesData }}')" 
                        ...
                        <img src="{{ $firstImage }}" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110">
                        ...
                        <div class="absolute inset-0 {{ $config['bg'] }} flex flex-col items-center justify-center text-white transition-all duration-300 backdrop-blur-[0px] group-hover:backdrop-blur-[2px]">
                            ...
                            <div class="absolute bottom-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 translate-y-2 group-hover:translate-y-0">
                                <span class="text-[8px] bg-white/20 px-2 py-0.5 rounded-full border border-white/30 backdrop-blur-md">Klik Detail</span>
                            </div>
                        </div>
                    </div>
                @empty
                    <div class="col-span-full py-16">
                        <div class="text-slate-300 mb-2 text-2xl">🛏️</div>
                        <p class="text-slate-400 text-sm italic">Belum ada data kamar untuk cabang ini.</p>
                    </div>
                @endforelse
            </div>
        </div>
        ...
        <div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-100">
            ...
            <p class="text-[10px] text-slate-500">{{ $branch->address }}</p>
            
            <div class="mt-6 w-full h-80 bg-slate-100 rounded-xl flex items-center justify-center border border-slate-200 relative overflow-hidden group shadow-inner">
                <span class="text-slate-400 text-xs font-medium italic z-10">Peta Lokasi Kost</span>
                <div class="absolute inset-0 bg-slate-200 opacity-20"></div>
            </div>
...
<div id="roomModal" class="hidden fixed inset-0 z-[999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm transition-opacity duration-300 opacity-0">
    <div class="bg-white w-full max-w-[380px] rounded-[2.5rem] shadow-2xl relative p-6 transform transition-all scale-95 duration-300" id="modalContent">
        <button onclick="closeModal()" class="absolute top-6 right-6 z-20 bg-white/90 backdrop-blur-md hover:bg-gray-100 p-2 rounded-full transition-colors shadow-sm">
        ...
        <div class="relative group overflow-hidden rounded-[1.5rem] mb-6 shadow-md border border-slate-50">
            <div id="modalSlider" class="flex overflow-x-auto snap-x snap-mandatory scroll-smooth no-scrollbar">
                {{-- JS Injected --}}
            </div>

            <button onclick="modalSlide('prev')" class="absolute left-2 top-1/2 -translate-y-1/2 bg-white/70 backdrop-blur-sm p-1.5 rounded-full text-slate-700 shadow-sm z-10">
            ...
            <button onclick="modalSlide('next')" class="absolute right-2 top-1/2 -translate-y-1/2 bg-white/70 backdrop-blur-sm p-1.5 rounded-full text-slate-700 shadow-sm z-10">
        ...
        <div class="grid grid-cols-2 gap-x-4 gap-y-6 mb-8 px-1">
            <div class="space-y-1">
                <p class="text-gray-400 text-[10px] uppercase tracking-widest font-bold">Nomor Kamar</p>
                <p id="modalRoomNum" class="font-bold text-slate-700 text-base">-</p>
            </div>
            <div class="space-y-1">
                <p class="text-gray-400 text-[10px] uppercase tracking-widest font-bold">Cabang</p>
                <p class="font-bold text-slate-700 text-base">{{ $branch->name }}</p>
            </div>
            <div class="space-y-1">
            ...
            <div class="space-y-1">
                ...
                <p id="modalPrice" class="font-black text-indigo-600 text-lg">-</p>
            </div>
        </div>

        <form id="bookingForm" method="GET">
            <button type="submit" id="bookingBtn" class="w-full bg-[#6366f1] hover:bg-[#4f46e5] text-white font-bold py-4 rounded-2xl transition-all shadow-xl shadow-indigo-200 active:scale-[0.98]">
...
<script>
    ...
    function openModal(id, nomor, harga, status, tipe, safeImagesData) {
        ...
        document.getElementById('modalRoomNum').innerText = nomor;
